...
%>
	...
	<script type="text/javascript" language="javascript">
		...
		function queryServerList2(obj){
			...
			$.pdialog.open("<%=basePath%>/zhxy/message/functionAdjust_server.action?syncType=2&types="+sign, "2", "<s:text name="synchronous" />",  {width:760,height:720});
		}
	</script>
...
<div class="pageContent">
	...
	<div class="panelBar">
		<ul class="toolBar">
			<li><a width="950" height="490" class="add" href="<%=basePath%>/cjwz/message/activity_addActivityIndex.action" target="dialog"><span><s:text name="new" /> <s:text name="increase" /></span></a></li>
			<li class="line">line</li>
			
<%-- 			<li><a class="icon" href="javascript:confirmRefresh({'url':'<%=basePath %>/cjwz/base/activity_reflash.action'})"><span><s:text name="refresh_the_activity" /></span></a></li>
 --%>		<%-- 	<li><a class="icon" width="770" height="360"    href="javascript:queryServerList2(this);"><span><s:text name="synchronous" /></span></a></li> --%>
	...
	<table class="list" width="100%" layoutH="42" style="table-layout: fixed;">
		<thead>
	    	<tr>
	    		<th width="20"><input type="checkbox" group="id2" class="checkboxCtrl"></th>
	    	...
	    	<tbody>
	    	<s:iterator value="activityList" var="oneRow"  status="offset">
	    		<tr >
	    			<td>
		    	    	<input type="checkbox" name="id2" value="${oneRow.activityId}"> 
		    	    </td>
		    	    <td>
		    	    	<s:property value="#oneRow.activityId"/>
		    	    </td>
		    	     <td>
		    	    <%-- 	<s:property value="#oneRow.type"/> --%><s:property value="#oneRow.type"/>
		    	    </td>
		    	    ...
		    	    <td class="tdwarp" title="<s:property value="#oneRow.rule"/>">
		    	   		 <s:property value="#oneRow.rule"/>
		    	    </td>
		    	     <td class="tdwarp" title="<s:property value="#oneRow.rule"/>">
		    	   		 <s:property value="#oneRow.reward"/>
		    	    </td>
		    	     <td class="tdwarp" title="<s:property value="#oneRow.btnName"/>">
		    	   		 <s:property value="#oneRow.btnName"/>
		    	    </td>
		    	     <td class="tdwarp" title="<s:property value="#oneRow.icon"/>">
		    	   		 <s:property value="#oneRow.icon"/>
		    	    </td>
		    	    <td class="tdwarp" title="<s:property value="openTime"/>">
		    	    	<s:property value="openTime"/>
		    	    	<%-- <s:date format="yyyy-MM-dd HH:mm:ss" name="#oneRow.openTime"/> --%>
		    	    </td>
		    	    <td class="tdwarp" title="<s:property value="#oneRow.startTime"/>">
		    	    	<s:property value="#oneRow.startTime"/></td>
		    	     <td class="tdwarp" title="<s:property value="#oneRow.endTime"/>">
		    	    	<%-- <s:date format="yyyy-MM-dd HH:mm:ss" name="#oneRow.overTime"/> --%>
		    	    	
		    	    	<s:property value="#oneRow.endTime"/>
		    	    	</td>
		    	   <%--   <td>
		    	   		 <s:property value="#oneRow.carrierOperator"/>
		    	    </td> --%>
		    	    <td> 
		    	    		 <a width="930" height="600" href="<%=basePath%>/cjwz/message/activity_updateActivityIndex.action?activity.activityId=${oneRow.activityId}" style="color:green" class="btnEdit" target="dialog">
					    	      	<span><s:text name="modify_the" /></span>
					    	      </a>
					 </td>
		    	    <td> 
		    	     		 <a href="javascript:confirmRefresh({'confirmMsg':'<s:text name="are_you_sure_you_want_to_delete_this_activity" />？','url':'<%=basePath %>/cjwz/message/activity_deleteActivity.action','data':{'activity.activityId':${oneRow.activityId}}});" style="color:green" class="btnDel" >
