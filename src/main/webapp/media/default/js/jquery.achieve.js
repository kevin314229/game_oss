	...
	$.fn.achieve.action = function(input, settings, itemArray, itemData, mouseDownOnSelect) {
		...
		function init(id) {
			if(!$('#'+id).length) {
				...
				list = $("<ul/>").appendTo(element).css({'text-align':'left',"overflow":"auto", "max-height": settings.height
					...
					input[0].selectedRow = itemData[Number($(evtTarget).attr('key'))];
		...
		function fillList() {
			...
			for (var i in itemData) {
				...
				//var li = $("<li/>").html(formatted).attr('key', i).addClass(i%2 == 0 ? "ac_even" : "ac_odd").appendTo(list)[0];
			}
			list.append(li);
			...
			if($.browser.msie && Number(jQuery.browser.version) < 7) {
				if(max < 10) {
					element.find('ul').css({'text-align':'left','height':'', "overflow":"auto", "max-height": settings.height});
				} else {
					element.find('ul').css({'text-align':'left',height: settings.height});
				}
			}
			element.show();
			list.scrollTop(0);
