...
<script src="<%=basePath%>/media/default/js/dwz.util.province.city.js" type="text/javascript"></script>
<script language="javascript">
	...
	where[1] = new comefrom("北京",
			"|东城|西城|崇文|宣武|朝阳|丰台|石景山|海淀|门头沟|房山|通州|顺义|昌平|大兴|平谷|怀柔|密云|延庆"); //欢迎来到站长特效网，我们的网址是www.zzjs.net，很好记，zz站长，js就是js特效，本站收集大量高质量js代码，还有许多广告代码下载。
	where[2] = new comefrom("上海",
			"|黄浦|卢湾|徐汇|长宁|静安|普陀|闸北|虹口|杨浦|闵行|宝山|嘉定|浦东|金山|松江|青浦|南汇|奉贤|崇明");//欢迎来到站长特效网，我们的网址是www.zzjs.net，很好记，zz站长，js就是js特效，本站收集大量高质量js代码，还有许多广告代码下载。
	...
	function select() {
		with (document.creator2.province) {
		...
		//document.creator2.newlocation.value = loca2 + loca4;
	} 
	function init() {
		with (document.creator2.province) {
			length = where.length;
	...
	$(function() {
		 $.provinceCity.initProvince($("#province"),$("#city"));
	});
</script>
...
<div class="pageHeader">
	...
	<s:form  rel="pagerForm" action="dudubear_pay" method="post" theme="simple"
		id="queryRecordForm" name="creator2" 
		onsubmit="return $(this).valid()&&navTabSearch(this);">
		<div class="searchBar">
			<div class="searchDiv_left">
				省份 <select style="width: 140px"  name="province" id="province">
				...
				城市 <select  name="city" id="city">
					...
					<option value="抚顺"></option>
					<option value="本溪"></option>
					<option value="丹东"></option>
					<option value="锦州"></option>
					<option value="营口"></option>
					<option value="阜新"></option>
					<option value="辽阳"></option>
					<option value="盘锦"></option>
					<option value="铁岭"></option>
					<option value="朝阳"></option>
					<option value="葫芦岛"></option>
				</select> 渠道ID:
				<s:select headerKey="0" headerValue="-请选择-"
					list="#{ '360':'360','dl':'当乐','xm':'小米'}"></s:select>
				SDK(平台码)：
				<s:textfield name="pt_code"></s:textfield>
...
<div class="pageContent">
	<table id="row" class="list" width="100%" layoutH="82">
	<thead>
		<tr class="even">
			<th>订单ID</th>
			<th>充值金额</th>
		...
		<tbody>
		<!-- [{"complete_time":"2014-07-10 15:07:12","pt_code":"wonder","pt_money":70,"pt_order":"4","rmb":100} -->
		<s:iterator value="jsonarrayArray" status="offsets" var="oneRow">
			<tr <s:if test="#offsets.even == true"> class="even" </s:if>
				...
				<td><s:property value="#oneRow.pt_order" /></td>
				<td><s:property value="#oneRow.amount" /></td>
				<td><s:property value="#oneRow.pt_number" /></td>
				<td><s:property value="#oneRow.complete_time" /></td>
				<td><s:property value="#oneRow.pt_code" /></td>
				<td><s:property value="#oneRow.opratorID" /></td>
				<td><s:property value="#oneRow.province" />
					<s:property value="#oneRow.city" /></td>
			</tr>
		</s:iterator>
		</tbody>
	</table>
