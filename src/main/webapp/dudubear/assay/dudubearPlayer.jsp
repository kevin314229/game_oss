...
<div class="pageHeader">
	...
	<div id="divLeft" style="height: 18px">
		<b>数据分析 ：用户查询</b> <span class="color-red"><s:property
				value="errorInfo" /></span> <span class="color-gr"><s:property
				value="successInfo" /></span>
	</div>
	<s:form rel="pagerForm"  action="dudubear_player" method="post" theme="simple"
		id="queryRecordForm" name="creator"
		onsubmit="return $(this).valid()&&navTabSearch(this);">
		<div class="searchBar">
			<div class="searchDiv_left">
				省份 <select style="width: 140px" onchange="select()" name="province">
				...
				</select> 城市 <select onchange="select()" name="city">
					<option value=""></option>
...
<div class="pageContent">
	<table id="row" class="list" width="100%" layoutH="82">
		...
		<tbody>
		<!-- [{"pay_nub":1,"pay_sum":70,"playerCreateTime":"2014-07-10 11:21:08"}]
		phoneNumber,opratorID, playerCreateTime,lastLoginTime, phoneType,province,city,c.pay_nub,c.pay_sum  -->
		<s:iterator value="jsonarrayArray" status="offsets" var="oneRow">
			<tr <s:if test="#offsets.even == true"> class="even" </s:if>
				...
				<td><s:property value="#oneRow.phoneNumber" /></td>
				<td><s:property value="#oneRow.opratorID" /></td>
				<td><s:property value="#oneRow.playerCreateTime" /></td>
				<td><s:property value="#oneRow.lastLoginTime" /></td>
				<td><s:property value="#oneRow.phoneType" /></td>
				<td><s:property value="#oneRow.province" /> <s:property
						value="#oneRow.city" /></td>
				<td><s:property value="#oneRow.pay_nub" /></td>
				<td><s:property value="#oneRow.pay_sum" /></td>
			</tr>
		</s:iterator>
		</tbody>
	</table>
