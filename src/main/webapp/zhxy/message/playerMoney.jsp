...
function detail(baseid,loginName,nickName){
	var url = "<%=basePath%>/zhxy/message/zhplayermoney_detail.action?playerMoneyDay.playerBaseId="+baseid+"&loginName="+loginName+"&nickName="+nickName;
...
<div class="pageHeader">
	<s:form action="zhplayermoney_index" method="post" id="queryForm" theme="simple" cssClass="pageForm required-validate" onsubmit="return $(this).valid()&&navTabSearch(this)">
		<div class="searchBar">
...
<div class="pageContent sortDrag" selector="h1" layoutH="50">
		<div class="panel">
			...
			<s:iterator value="ossPlayerMoneys" status="offsets" var="item">
				<s:form action="zhplayermoney_modify" method="post" theme="simple" onsubmit="return $(this).valid()&&navTabSearch(this)">
				<tr id="item<s:property value="#offsets.index"/>">
					...
					<s:if test="ossPlayerMoneys.size==1">
						<td>
							<a class="button" href="javascript:modifyBaseProperty('<%=basePath%>/zhxy/message/zhplayermoney_modify.action?','item<s:property value="#offsets.index"/>');"><span><s:text name="modification" /></span></a>
						</td>
					</s:if>
					<s:if test="ossPlayerMoneys.size>1">
						<td>
		...
		<div class="panel">
			<table class="list" width="100%" >
				...
				<s:iterator value="ossItemNumbers" status="offsets" var="item">
					...
					<s:form action="zhplayermoney_itemmodify" method="post" theme="simple" onsubmit="return $(this).valid()&&navTabSearch(this)">
						<td>
						...
						<td>
							<a class="button" href="javascript:modifyBaseProperty('<%=basePath%>/zhxy/message/zhplayermoney_itemmodify.action?','itemNumber<s:property value="#offsets.index"/>');"><span><s:text name="modification" /></span></a>
						</td>
					</s:form>
					</tr>
				</s:iterator>
			...
			<table class="list" width="100%" >
				...
				<s:iterator value="ossEquips" status="offsets" var="item">
					<tr>
						...
						<td>
							<s:form action="zhplayermoney_equipmodify" method="post" theme="simple" onsubmit="return $(this).valid()&&navTabSearch(this)">
