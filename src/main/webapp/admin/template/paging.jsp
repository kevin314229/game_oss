...
<div class="panelBar" >
			<div class="pages">
				...
				<select name="numPerPage" onchange="navTabPageBreak({numPerPage:this.value,pageNum:1})">
					...
					<option value="500" <s:if test="#request.onePageNum==500">selected</s:if>>500</option>
				</select>
				<span><s:text name="article"/>,<s:text name="total"/><s:property value="allNum"/><s:text name="article"/></span>
			</div>
			<div class="pagination" targetType="navTab" totalCount="${allNum}" numPerPage="${onePageNum}" pageNumShown="10" currentPage="${currPageNO}"></div>
</div>