...
<html>
	...
	<script type="text/javascript" language="javascript">
        ...
        function deleteMsg(msgID) {
			if (confirm("你确定要删除该项吗？")){
				deleteTaskLoopForm.loopId.value = msgID;
				deleteTaskLoopForm.submit();
			}
		}
	</script>
  </head>
  ...
  <div id="container">
    ...
    <div id="divQuery">
	    <s:form action="queryBaseTaskLoop" method="post" theme="simple" id="queryTaskLoopForm">
	    <s:hidden name="currPageNO" id="currPageNO"></s:hidden>
	    	    	任务ID：<s:textfield name="loopId" size="16"></s:textfield>
	    	    	<input type="button" style="cursor:hand" name="stat" value="查询" onclick="statPayInfo();"/>
	    	    	<input name="add" type="button" class="button_02" value="新 增" onclick="window.open('<%=basePath%>/admin/dictionary/addBaseTaskLoop.jsp','_self');" >
	    </s:form>
	</div>
	<div id="divLeft">
		<s:form action="deleteBaseTaskLoop" method="post" theme="simple" id="deleteTaskLoopForm">
		<s:hidden name="loopId" id="loopId"></s:hidden>
		</s:form>
	</div>
	<div id="topPager"></div>
	<div id="divList">
	    <table id="row">
	    	...
	    	<s:iterator value="baseTaskLoopList" var="onePay" status="offset">
	    		<tr >
		    	    <td>
						<s:property value="#onePay.loopId"/>
		    	    </td>
		    	    <td>
						<s:property value="#onePay.recvNumber"/>
		    	    </td>
		    	    <td>
		    	    ...
		    	    <td>
						 <A href='javascript:deleteMsg("<s:property value="#onePay.loopId"/>")' style="color:green">删除</A>
						 <a href="<%=basePath%>/admin/skill/editBaseTaskLoop.action?loopId=${onePay.loopId }" style="color:green">修改</a>
		    	    </td>
		    	</tr>
	    	</s:iterator>
	    </table>
