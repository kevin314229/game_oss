...
<html>
  <head>
    ...
    <script type="text/javascript" language="javascript">
    	...
    	var jsonEquip = '<s:property value="equipJson"/>';
    	var jsonProp = '<s:property value="propJson"/>';
    	jsonEquip=jsonEquip.replace(new RegExp('&quot;','g'),'"');
    	jsonProp=jsonProp.replace(new RegExp('&quot;','g'),'"');
    	var equip = eval(jsonEquip);
    	var prop = eval(jsonProp);
    	var tb1 = document.getElementById("tb1");
    	var tb2 = document.getElementById("tb2");
    	var sHtml;
    	if(jsonEquip!=null&&''!=jsonEquip){
    	document.getElementById("eq1").value=equip[0].equipId;
    	document.getElementById("rt1").value=equip[0].rate;
    	for(k=1;k<equip.length;k++){
    		tb1.insertRow(tb1.rows.length);
    		tb1.rows.item(tb1.rows.length-1).insertCell(0);
    		tb1.rows.item(tb1.rows.length-1).cells.item(0).colSpan="2";
    		sHtml=(j++)+'、装备ID：<input type="text" value=\"'+equip[k].equipId+'\" id=\"eq'+(j-1)+'\" name=\"eq'+(j-1)+'\">'+'几率：<input type="text" value=\"'+equip[k].rate+'\" id=\"rt'+(j-1)+'\" name=\"rt'+(j-1)+'\">';
    		tb1.rows.item(tb1.rows.length-1).cells.item(0).innerHTML=sHtml;
    		i++;
    	}
    	}
    	if(jsonProp!=null&&''!=jsonProp){
    	document.getElementById("pp1").value=prop[0].propId;
    	document.getElementById("prt1").value=prop[0].rate;
    	document.getElementById("num1").value=prop[0].number;
    	for(k=1;k<prop.length;k++){
    		tb2.insertRow(tb2.rows.length);
    		tb2.rows.item(tb2.rows.length-1).insertCell(0);
    		tb2.rows.item(tb2.rows.length-1).cells.item(0).colSpan="2";
    		sHtml=(j2++)+'、道具ID：<input type="text" value=\"'+prop[k].propId+'\" id=\"pp'+(j2-1)+'\" name=\"pp'+(j2-1)+'\" size=\"10\">'+'数量：<input type="text" value=\"'+prop[k].number+'\" id=\"num'+(j2-1)+'\" name=\"num'+(j2-1)+'\" size=\"10\">'+'几率：<input type="text" value=\"'+prop[k].rate+'\" id=\"prt'+(j2-1)+'\" name=\"prt'+(j2-1)+'\" size=\"10\">';
    		tb2.rows.item(tb2.rows.length-1).cells.item(0).innerHTML=sHtml;
    		i2++;
    	}
    	}
    		$('#a1').hide();
  ...
  <body>
	...
	<div id="divLeft">
		新增掉落：
		<s:form action="updateBaseDrop" namespace="/admin/skill" method="post" theme="simple" id="addDropForm">
		...
		<tr>
		<td  style="background-color:#7dc4e6;">掉落详情</td><td>
			<table id="tb1">
			<tr>
			<td>
			1、装备ID：
		<s:textfield name="eq1" id="eq1"/>几率：<s:textfield name="rt1" id="rt1"/><a href="javascript:addRow(1);">增加</a>
		...
		<input type="button" value="确认修改" onClick="addJson();">
